...
import { GetProfiler } from './__generated__/GetProfiler';
import { GET_PROFILER } from './get_profiler';

@Component({
  selector: 'app-profiler',
  templateUrl: './profiler.component.html',
  styleUrl: './profiler.component.less'
})
export class ProfilerComponent implements OnInit {
  ...
  ngOnInit(){
    this.apollo.query<GetProfiler>({
      ...
      result => {
        this.pdfSrc = result?.data?.core?.product?.profiler?.url ?? '';
      },
      error => {
